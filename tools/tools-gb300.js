/*
  ...
  Dteyn's GB300 Tools Shared JS Library
  =====================================

  Based on VonMillhausen's tools.js v1.6 (commit: a6feed8)
...
function getFirmwareHash(data) {
  ...
  if (dataCopy.length > 7200000) {
    ...
    // Location: Approximately 0x5f2880 (about 75% of the way through the file)
	...
	// Next identify the location of m_st7789_init and zero it out, in case the user has
	// applied the patch to use a SF2000 IPS panel in the GB300.
	// Location: Approximately 0x6cbfa4
	var m_st7789initOffset = findSequence([0x10, 0x3A, 0x66, 0x80, 0x18, 0x3A, 0x66, 0x80, 0x20, 0x3A, 0x66, 0x80, 0x28, 0x3A, 0x66, 0x80], dataCopy);
	if (m_st7789initOffset > -1) {
      var m_st7789initStart = m_st7789initOffset + 16;
	  for (var i = m_st7789initStart; i < (m_st7789initStart + 156); i++) {
		  dataCopy[i] = 0x00;
	  }
	}
	else {
	...
	// In SF2000, we searched for "bad_exception", however in GB300, there is extra data after "bad_exception".
	// Instead, we need to search for "fundamental_type_info" + add 2 to find boot logo offset
    // Location: Approximately 0x670a54 to 0x69b253 - length 0x2a800 (174080 bytes)
    var fundamentalTypeInfoOffset = findSequence([0x66, 0x75, 0x6E, 0x64, 0x61, 0x6D, 0x65, 0x6E, 0x74, 0x61, 0x6C, 0x5F, 0x74, 0x79, 0x70, 0x65, 0x5F, 0x69, 0x6E, 0x66, 0x6F, 0x45, 0x00], dataCopy);
    if (fundamentalTypeInfoOffset > -1) {
      var bootLogoStart = fundamentalTypeInfoOffset + 23;
      for (var i = bootLogoStart; i < (bootLogoStart + 174080); i++) {
        dataCopy[i] = 0x00;
      }
    }
    else {
...
function knownHash(hash) {
  switch (hash) {
    case "b2083abce1d8fc81136c90e068729809da6a66ada0307ac3fce13ba261c8b5e5":
      return "12.15";
